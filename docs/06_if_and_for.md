...
它叫做 `function`，中文叫做「函數」或是「函式」，下面是關於它的三點小知識。

### 什麼是 `function`？
  
你可以把它想成是一個「打包好的小任務」，這個小任務可以重複使用，流程通常是這樣：

  - 丟一些資料進去  
  - 它在裡面做一堆事情  
  - 最後把結果丟回來  

<pre><code>結果 = 小任務(你丟進去的東西)</code></pre>

在一開始的例子中，`normalize_text`就是這個小任務的名字，絕大多數的時候你都能夠從名字看出它的任務是什麼，`text`則是你丟進去的資料，而`return text`則是最後你會拿到的結果。

**90% 的情況下，只要知道這三件事就很夠用了：**
